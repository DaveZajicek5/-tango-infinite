(() => {
  ...
  function trans(L,t){
    const g=[...new Set(L.g.map(i=>cellT(i,t)))], e=[], seen=new Set();
    for(const s of L.e){ const [a,b]=ends(s), q=edgeT(cellT(a,t),cellT(b,t)); if(!q)continue; const k=`${q.r},${q.c},${q.d}`; if(!seen.has(k)){seen.add(k);e.push(q);} }
    return {n:L.n,g,e};
  }
  ...
  function score(p){ return 120-Math.abs(p.givenCount-8)*5-Math.min(Math.abs(p.signCount-4),Math.abs(p.signCount-6),Math.abs(p.signCount-8))*6-Math.abs(emptyLines(p.givens)-4)*4+hiddenRate(p)*20-(p.profile?.abstractLine||0)*5; }
  ...
  function mutate(L){ const g=new Set(L.g), e=L.e.slice(), cells=[...Array(36).keys()]; if(g.size>4&&Math.random()<.55)g.delete(pick([...g])); if(g.size<14)g.add(pick(cells)); const gs=new Set(g), used=new Set(e.map(x=>`${x.r},${x.c},${x.d}`)); for(const ed of shuf(allEdges())){ if(e.length>=12||Math.random()<.35)break; const k=`${ed.r},${ed.c},${ed.d}`; if(!used.has(k)&&!touch(ed,gs)){used.add(k);e.push(ed);} } return {n:L.n,g:[...g],e}; }
  function skeleton(mut=false){ const bank=parsePack(); if(!bank.length)return null; return best(()=>{const base=pick(bank); const L=mut?mutate(trans(base,rint(0,7))):trans(base,rint(0,7)); return make(L,sol(),mut?`LI mutant #${base.n}`:`LI skeleton #${base.n}`,{generator:mut?'mutant':'skeleton',source:'li',archiveId:base.n});},mut?220:150,{hid:mut?.72:.85}); }
  ...
  function archive(){ const bank=parsePack(); if(!bank.length)return null; return best(()=>{const L=pick(bank); return make(L,L.s,`Archive #${L.n}`,{generator:'archive',source:'archive',archiveId:L.n});},80,{hid:0,abs:9,rel:30}); }
  function mode(){ try{const m=localStorage.getItem('tangoGenerator'); if(modes.some(x=>x.id===m))return m;}catch(_){} return 'hybrid'; }
  function gen(m=mode()){ if(m==='original')return oldGenerate(); if(m==='archive')return archive()||oldGenerate(); if(m==='skeleton')return skeleton(false)||oldGenerate(); if(m==='mutant')return skeleton(true)||skeleton(false)||oldGenerate(); if(m==='grammar')return grammar()||skeleton(false)||oldGenerate(); const r=Math.random(); return r<.6?(skeleton(false)||grammar()||oldGenerate()):r<.85?(skeleton(true)||skeleton(false)||oldGenerate()):(grammar()||skeleton(false)||oldGenerate()); }
